...
if (!app_verify_csrf('cambiar_password_profesor', $_POST['csrf_token'] ?? '')) {
...
if ($actual === '' || strlen($nuevo) < 10) {
    app_flash('profesor_error', 'Captura tu contrasena actual y una nueva de al menos 10 caracteres.');
    app_redirect('seguridad.php');
}

if (!hash_equals($nuevo, $confirmacion)) {
    app_flash('profesor_error', 'La confirmacion de contrasena no coincide.');
    app_redirect('seguridad.php');
}
...
try {
    ...
    if (!$usuario || !password_verify($actual, $usuario['password_hash'])) {
        app_flash('profesor_error', 'La contrasena actual no es correcta.');
        app_redirect('seguridad.php');
    }
    ...
    app_audit('profesor.password.actualizar', 'usuario', $_SESSION['usuario_id']);
    $pdo->commit();

    app_flash('profesor_message', 'Contrasena actualizada correctamente.');
    app_redirect('seguridad.php');
    ...
    app_flash('profesor_error', 'No fue posible actualizar la contrasena.');
    app_redirect('seguridad.php');
}
